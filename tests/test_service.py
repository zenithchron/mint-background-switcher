...
def test_polaroid_span_uses_configured_count_across_combined_desktop(monkeypatch, tmp_path: Path):
    _setup_profile(monkeypatch, tmp_path)
    cfg = service.load_config()
    profile = cfg.get_profile("P")
    profile.mode = "polaroid"
    profile.polaroid_count = 7
    profile.polaroid_span = True
    save_config(cfg)
    captured = {}

    def fake_compose_polaroid(monitors, images_by_monitor, output_path, **kwargs):
        captured["monitors"] = [monitor.name for monitor in monitors]
        captured["images_by_monitor"] = {name: list(paths) for name, paths in images_by_monitor.items()}
        captured.update(kwargs)
        Path(output_path).parent.mkdir(parents=True, exist_ok=True)
        Path(output_path).write_bytes(b"spanning-polaroid")
        return Path(output_path)
    ...
    assert captured["images_by_monitor"] == {"A": result.images}
    assert captured["span"] is True
    assert draw_counts == [("profile:P:polaroid", 7)]
...
def test_saturate_effect_dry_run_preserves_sources_state_and_black_screen(
    monkeypatch, tmp_path: Path
):
    _setup_profile(monkeypatch, tmp_path)
    cfg = service.load_config()
    cfg.get_profile("P").mode = "same"
    cfg.get_profile("P").effect = "saturate"
    save_config(cfg)
    original = RuntimeState(
        paused=True,
        black_screen=True,
    ...
    assert len(result.images) == 1
    with Image.open(result.images[0]) as selected:
        expected = ImageEnhance.Color(selected.convert("RGB")).enhance(1.5)
        expected_pixel = expected.getpixel((0, 0))
    with Image.open(result.wallpaper) as wallpaper:
        assert wallpaper.mode == "RGB"
        assert wallpaper.size == (220, 80)
