...
def test_settings_effect_menu_exposes_saturate_and_applies_selection(
    monkeypatch, tmp_path
):
    monkeypatch.setenv("MBS_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.setenv("MBS_CACHE_DIR", str(tmp_path / "cache"))
    ...
    try:
        ...
        monkeypatch.setattr(
            ...
            lambda profile, **_kwargs: (
                applied_profiles.append(profile)
                or SimpleNamespace(wallpaper="saturate-preview.png")
            ),
        )
        monkeypatch.setattr(
            settings_ui.messagebox,
        ...
        assert str(app.effect_menu).startswith(str(app.general_tab))
        assert app.effect_menu.winfo_ismapped()
        assert app.effect_menu.winfo_width() > 1
        menu.invoke(labels.index("saturate"))
        assert app.effect_var.get() == "saturate"
        app._apply_next()
        _pump_until(app, lambda: not app._apply_busy)
        assert saved_effects == ["saturate"]
        assert applied_profiles == ["Default"]
        assert messages and messages[-1][0] == "Applied"
        ...
        monkeypatch.setattr(
            ...
            "switch_once",
            lambda _profile, **_kwargs: (_ for _ in ()).throw(
                RuntimeError("saturate preview failed")
            ),
        )
        app._apply_next()
        _pump_until(app, lambda: not app._apply_busy)
        assert saved_effects == ["saturate", "saturate"]
        assert errors == [("Apply failed", "saturate preview failed")]
    finally:
        app.destroy()
...
def test_settings_effect_menu_exposes_random_and_applies_selection(
    monkeypatch, tmp_path
):
    monkeypatch.setenv("MBS_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.setenv("MBS_CACHE_DIR", str(tmp_path / "cache"))
    ...
    try:
        ...
        monkeypatch.setattr(
            ...
            lambda profile, **_kwargs: (
                applied_profiles.append(profile)
                or SimpleNamespace(wallpaper="random-preview.png")
            ),
        )
        monkeypatch.setattr(
            settings_ui.messagebox,
        ...
        assert app.effect_menu.winfo_ismapped()
        assert app.effect_menu.winfo_width() > 1
        menu.invoke(labels.index("random"))
        assert app.effect_var.get() == "random"
        app._apply_next()
        _pump_until(app, lambda: not app._apply_busy)
        assert saved_effects == ["random"]
        assert applied_profiles == ["Default"]
        assert messages and messages[-1][0] == "Applied"
        ...
        monkeypatch.setattr(
            ...
            "switch_once",
            lambda _profile, **_kwargs: (_ for _ in ()).throw(
                RuntimeError("random preview failed")
            ),
        )
        app._apply_next()
        _pump_until(app, lambda: not app._apply_busy)
        assert saved_effects == ["random", "random"]
        assert errors == [("Apply failed", "random preview failed")]
    finally:
        app.destroy()
...
@pytest.mark.skipif(not os.environ.get("DISPLAY"), reason="requires a graphical display or Xvfb")
def test_settings_mode_menu_exposes_polaroid_and_applies_selection(monkeypatch, tmp_path):
    monkeypatch.setenv("MBS_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.setenv("MBS_CACHE_DIR", str(tmp_path / "cache"))
    monkeypatch.setattr(settings_ui, "detect_monitors", lambda: [])
    app = settings_ui.SettingsApp()
    try:
        ...
        def capture_config(config):
            profile = config.get_profile("Default")
            saved_modes.append(profile.mode)
            saved_polaroid_options.append(
                (profile.polaroid_count, profile.polaroid_size, profile.polaroid_span, profile.polaroid_tilt)
            )
        ...
        monkeypatch.setattr(
            ...
            "switch_once",
            lambda profile, **_kwargs: applied_profiles.append(profile)
            or SimpleNamespace(wallpaper="/tmp/polaroid-preview.png"),
        )
        monkeypatch.setattr(
            settings_ui.messagebox,
            "showinfo",
        ...
        assert app.polaroid_tilt_checkbutton.winfo_ismapped()
        assert app.polaroid_tilt_checkbutton.cget("text") == "Randomly tilt prints"
        assert app.polaroid_tilt_var.get() is True
        assert (
            app.polaroid_tilt_checkbutton.winfo_rootx() + app.polaroid_tilt_checkbutton.winfo_width()
            <= app.polaroid_options.winfo_rootx() + app.polaroid_options.winfo_width()
        )
        assert (
            app.polaroid_tilt_checkbutton.winfo_rooty() + app.polaroid_tilt_checkbutton.winfo_height()
            <= app.polaroid_options.winfo_rooty() + app.polaroid_options.winfo_height()
        )
        ...
        monkeypatch.setattr(
            ...
            "switch_once",
            lambda _profile, **_kwargs: SimpleNamespace(
                wallpaper="/tmp/polaroid-black-fallback.png",
                action="black-fallback",
            ),
        )
        app._apply_next()
        ...
        assert saved_polaroid_options == [(7, 0.8, True, False), (7, 0.8, True, False)]
        assert messages[-1][0] == "Safe black fallback"
        assert "No usable source images" in messages[-1][1]
        ...
        monkeypatch.setattr(
            settings_ui,
            "switch_once",
            lambda _profile, **_kwargs: (_ for _ in ()).throw(RuntimeError("polaroid preview failed")),
        )
        app._apply_next()
        _pump_until(app, lambda: not app._apply_busy)
        assert saved_modes == ["polaroid", "polaroid", "polaroid"]
        assert saved_polaroid_options == [
            (7, 0.8, True, False),
            (7, 0.8, True, False),
            (7, 0.8, True, False),
        ]
